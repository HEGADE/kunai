...
func ConvertCodexResponseToClaude(_ context.Context, _ string, originalRequestRawJSON, _ []byte, rawJSON []byte, param *any) [][]byte {
	...
	case "response.created":
		...
		output = AppendSSEEventBytes(output, "message_start", template, 2)
	...
	case "response.reasoning_summary_text.delta":
		...
		output = AppendSSEEventBytes(output, "content_block_delta", template, 2)
	...
	case "response.completed", "response.incomplete":
		...
		output = AppendSSEEventBytes(output, "message_delta", template, 2)
		output = AppendSSEEventBytes(output, "message_stop", []byte(`{"type":"message_stop"}`), 2)
...
func codexStreamErrorToClaudeError(rootResult gjson.Result) []byte {
	...
	return AppendSSEEventBytes(nil, "error", out, 2)
}
	...
	if output := responseData.Get("output"); output.Exists() && output.IsArray() {
		output.ForEach(func(_, item gjson.Result) bool {
			...
			case "function_call":
				...
				toolBlock, _ = sjson.SetBytes(toolBlock, "id", shortenCodexCallIDIfNeeded(SanitizeClaudeToolID(item.Get("call_id").String())))
...
func appendCodexFunctionCallStart(output []byte, originalRequestRawJSON []byte, callID, name string, blockIndex int) []byte {
	...
	template, _ = sjson.SetBytes(template, "content_block.id", shortenCodexCallIDIfNeeded(SanitizeClaudeToolID(callID)))
	template, _ = sjson.SetBytes(template, "content_block.name", resolveCodexClaudeToolUseName(originalRequestRawJSON, name))
	return AppendSSEEventBytes(output, "content_block_start", template, 2)
}
...
func appendCodexFunctionCallArgumentDelta(output []byte, partialJSON string, blockIndex int) []byte {
	...
	return AppendSSEEventBytes(output, "content_block_delta", template, 2)
}

func appendCodexFunctionCallStop(output []byte, blockIndex int) []byte {
	template := []byte(`{"type":"content_block_stop","index":0}`)
	template, _ = sjson.SetBytes(template, "index", blockIndex)
	return AppendSSEEventBytes(output, "content_block_stop", template, 2)
}
...
func ClaudeTokenCount(_ context.Context, count int64) []byte {
	return ClaudeInputTokensJSON(count)
}
...
func startCodexTextBlock(params *ConvertCodexResponseToClaudeParams) []byte {
	...
	return AppendSSEEventBytes(nil, "content_block_start", template, 2)
}
...
func stopCodexTextBlock(params *ConvertCodexResponseToClaudeParams) []byte {
	...
	return AppendSSEEventBytes(nil, "content_block_stop", template, 2)
}
...
func finalizeCodexThinkingBlock(params *ConvertCodexResponseToClaudeParams) []byte {
	...
	if params.ThinkingSignature != "" {
		...
		output = AppendSSEEventBytes(output, "content_block_delta", signatureDelta, 2)
	}
	...
	output = AppendSSEEventBytes(output, "content_block_stop", contentBlockStop, 2)
