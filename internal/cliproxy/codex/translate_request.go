...
func ConvertClaudeRequestToCodex(modelName string, inputRawJSON []byte, _ bool) []byte {
	...
	inputItems := NewRawArrayItems(rootResult.Get("messages.#").Int())
	...
	if systemsResult.Exists() {
		...
		if len(contentItems) > 0 {
			...
			message, _ = sjson.SetRawBytes(message, "content", JoinRawArray(contentItems))
	...
	if messagesResult.IsArray() {
		...
		for i := 0; i < len(messageResults); i++ {
			...
			if messageRole == "system" {
				if reminderText, ok := ClaudeMessageSystemReminderText(messageResult.Get("content")); ok {
					...
					case "tool_result":
						...
						if contentResult.IsArray() {
							...
							if len(toolResultContentItems) > 0 {
								functionCallOutputMessage, _ = sjson.SetRawBytes(functionCallOutputMessage, "output", JoinRawArray(toolResultContentItems))
							} else {
	...
	// Convert budget_tokens to reasoning.effort.
	...
	if thinkingConfig := rootResult.Get("thinking"); thinkingConfig.Exists() && thinkingConfig.IsObject() {
		...
		case "adaptive", "auto":
			...
			if effort != "" {
				...
				reasoningEffort = string(LevelXHigh)
			}
		case "disabled":
	...
	if toolsResult.IsArray() {
		template, _ = sjson.SetRawBytes(template, "tools", JoinRawArray(toolItems))
	}
	template = SetRawArrayItems(template, "input", inputItems)

	return template
}

func codexClaudeTargetAcceptsGrokSignature(modelName string) bool {
	baseModel := strings.ToLower(strings.TrimSpace(ParseSuffix(modelName).ModelName))
