<script lang="ts">
  ...
  // Reverting used to live here too, as a bare "Revert - sure?" second tap. It is
  // one control now, in the turn footer, and it asks with git's own answer instead:
  // a revert resets the WHOLE repository, so a confirmation attached to a card
  // listing this turn's files implied a scope it does not have.
